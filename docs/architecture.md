...
**Pre-flow data load (`app/check-in/page.tsx`):** Before rendering `CheckInFlow`, the check-in page fetches `GET /api/users`, `GET /api/check-ins?date={yesterday}`, and `GET /api/recommendations?date={yesterday}` in parallel to populate the "What did you do yesterday?" step. The "Láyo's suggested alternative" option card is only shown when yesterday's recommendation type was `modify` or `rest`; it is hidden for `as_written` (planned and suggested are identical) and when no recommendation exists. The heading for that card is derived from yesterday's recommendation via `getRecommendationHeading` in `lib/recommendation.ts` (shared with `RecommendationView`), which maps `rest` to a fixed heading and falls back to `modificationDetail` for `modify`.

...
**Pre-flow data load (`app/check-in/page.tsx`):** Before rendering `CheckInFlow`, the check-in page fetches `GET /api/users`, `GET /api/check-ins?date={yesterday}`, and `GET /api/recommendations?date={yesterday}` in parallel to populate the "What did you do yesterday?" step, including the "Láyo's suggested workout" option. The heading shown for that option is derived from yesterday's recommendation via `getRecommendationHeading` in `lib/recommendation.ts` (shared with `RecommendationView`), which maps `as_written`/`rest` to a fixed heading and falls back to `modificationDetail` for `modify`. If no recommendation exists for yesterday, the option shows "No suggestion on record".
...
**`TextInput`/`TextArea` font-size has a hard floor of 16px, with no smaller variant.** iOS Safari auto-zooms the viewport on focus for any input under 16px. LAYO-62 fixed this once across every input in the app; LAYO-128's code review response reintroduced a 13px `small` variant on `TextInput` for `/restore` specifically to fit a pasted UUID on one line, which reintroduced the same zoom bug (LAYO-132), and appears to also have caused the standalone PWA to intermittently fall back to showing Safari browser chrome and a residual layout shift on the page navigated to afterward. LAYO-132 removed the `small` variant entirely rather than just unusing it, so a smaller-than-16px input can't be reintroduced by a future call site without editing the shared component itself. This means the original LAYO-128 review request (fit a full pasted `deviceId` on one line without the input growing) is not fully satisfiable at 16px in the available width; long values may need horizontal scroll within the input or line-wrapping, whichever a future design pass prefers.

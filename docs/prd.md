...
- Tapping the planned workout value in the summary card expands it to show the full, untruncated text (no visible button styling). Tapping it again collapses it back to the truncated single-line presentation. This is the only summary row with this behavior.
